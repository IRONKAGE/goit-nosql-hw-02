...
12. **Семантика vs BM25 (Правило):** `BM25` слід використовувати для точних ідентифікаторів, імен авторів, специфічних хімічних формул чи абревіатур (SKU товарів). `Semantic` ідеальний для концепцій, ідей, питань користувача (QA) та перефразувань.
...
14. **Математика L2 та Cosine (Невідповідна метрика):** Доведемо зв'язок L2 та Cosine для одиничних векторів $A$ та $B$ ($||A||=1, ||B||=1$):
